...
from attacksplitnn2.splitnn import Client, Server, SplitNN
from attacksplitnn2.attack import Black_Box_Model_Inversion
from attacksplitnn2.utils import DataSet
...
DIR_PATH = 'C:/Users/DELL/FL-SL/4/data/reshape_ldp0.1'
...
trainset, testset = torch.utils.data.random_split(dataset, [train_dataset_size, test_dataset_size])
...
victim_idx = random.sample(range(len(trainset)), k=1400)
victim_train_idx = victim_idx[:1000]
attack_idx = victim_idx[1000:]
victim_test_idx = random.sample(range(len(testset)), k=15)


len(victim_train_idx)
...
len(victim_test_dataset)



victim_train_dataloader = torch.utils.data.DataLoader(victim_train_dataset, batch_size=64, shuffle=True, drop_last=True)
attack_dataloader = torch.utils.data.DataLoader(attack_dataset, batch_size=64, shuffle=True, drop_last=True)
...
for data in victim_train_dataloader:
    img = data['image']
    label = data['label']
...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, num_classes=1000, expansion=4,
        ...
        self.patch_proj = nn.Sequential(
            nn.Conv2d(1, dim_stem, 7, patch_size, 3),

            LayerNormProxy(dim_stem)
        ) if use_conv_patches else nn.Sequential(
            nn.Conv2d(1, dim_stem, patch_size, patch_size, 0),
...
class Attacker(nn.Module):
    def __init__(self):
        ...
        self.fla = nn.Flatten()
        self.ln1 = nn.Linear(16384, 15000)
        self.ln2 = nn.Linear(15000, 12288)
...
for i in range(1, 16):
    plt.subplot(3, 5, i)
    after_transforms = np.transpose(data[i - 1], (1, 2, 0))
    plt.imshow(after_transforms, cmap=plt.get_cmap('gist_gray'))
...
for i in range(1, 16):
    plt.subplot(3, 5, i)
    after_transforms = np.transpose(attack_result[i - 1], (1, 2, 0))
    plt.imshow(after_transforms, cmap=plt.get_cmap('gist_gray'))
plt.show()
...
for i in range(1, 16):
    plt.subplot(3, 5, i)
    after_transforms = np.transpose(data[i - 1], (1, 2, 0))  # 转换通道数
...
for i in range(1, 16):
    plt.subplot(3, 5, i)
    after_transforms = np.transpose(attack_result[i - 1], (1, 2, 0))  # 转换通道数
    plt.imshow(after_transforms, cmap=plt.get_cmap('gist_gray'))  # cmap='gist_gray'
plt.show()
